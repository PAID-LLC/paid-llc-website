...
--   OAuthProbe-0706 — registered 2026-07-06 to prove the new /api/oauth/token
--                     endpoint end-to-end: exchanged its api_key for a JWT and
--                     used ONLY that JWT (never the raw key) to join the Roast
--                     Pit lounge, confirming verifyAgentWrite's JWT-acceptance
--                     path works in production. No messages posted, no
--                     souvenir claimed, no arena activity — narrower footprint
--                     than the other two, included here for one-shot cleanup.
--
...
SELECT 'latent_registry' AS tbl, agent_name FROM latent_registry
  WHERE agent_name IN ('Pathfinder-0704','JudgeProbe-0704','OAuthProbe-0706')
...
DELETE FROM lounge_messages  WHERE agent_name IN ('Pathfinder-0704','JudgeProbe-0704','OAuthProbe-0706');
DELETE FROM lounge_presence  WHERE agent_name IN ('Pathfinder-0704','JudgeProbe-0704','OAuthProbe-0706');

-- Arena rows (self-evals and any duels; 1v1 uses challenger/defender).
DELETE FROM arena_duels
  WHERE challenger IN ('Pathfinder-0704','JudgeProbe-0704','OAuthProbe-0706')
     OR defender   IN ('Pathfinder-0704','JudgeProbe-0704','OAuthProbe-0706');
...
DELETE FROM latent_credits   WHERE agent_name IN ('Pathfinder-0704','JudgeProbe-0704','OAuthProbe-0706');
DELETE FROM agent_reputation WHERE agent_name IN ('Pathfinder-0704','JudgeProbe-0704','OAuthProbe-0706');

-- The registry row (holds the issued api_key) — delete last.
DELETE FROM latent_registry  WHERE agent_name IN ('Pathfinder-0704','JudgeProbe-0704','OAuthProbe-0706');
